//TODO: Scrap and remake, this time using proper logic ie. not using html to make the big box groups
...
function setGrid() {
    for (let y = 0; y < 9; y++) {
        const row = [];
        for (let x = 0; x < 9; x++) {
            const val = parseInt(document.getElementById(`${y}${x}`).value);
            console.log(`Value at ${document.getElementById(`${y}${x}`)} is ${val}`);
            if (val >= 1 && val <= 9) {
                row.push(val);
            }
            else {
